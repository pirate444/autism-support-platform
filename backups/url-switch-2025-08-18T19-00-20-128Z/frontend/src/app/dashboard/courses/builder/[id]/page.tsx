...
export default function CourseBuilderPage() {
  ...
  useEffect(() => {
    ...
    axios.get(`http://localhost:8080/api/courses/${courseId}`, { headers: { Authorization: `Bearer ${token}` } })
  ...
  useEffect(() => {
    if (!courseId) return;
    const token = localStorage.getItem("token");
    axios.get(`http://localhost:8080/api/course-sections/course/${courseId}`, { headers: { Authorization: `Bearer ${token}` } })
      .then(res => {
        setSections(res.data);
        // Fetch lessons for each section
        res.data.forEach(section => {
          axios.get(`http://localhost:8080/api/course-lessons/section/${section._id}`, { headers: { Authorization: `Bearer ${token}` } })
  ...
  const handleSectionSubmit = async (e) => {
    ...
    try {
      if (editingSectionId) {
        await axios.put(`http://localhost:8080/api/course-sections/${editingSectionId}`, { ...sectionForm }, { headers: { Authorization: `Bearer ${token}` } });
      } else {
        console.log("Creating new section with data:", { ...sectionForm, courseId });
        const response = await axios.post(`http://localhost:8080/api/course-sections/`, { ...sectionForm, courseId }, { headers: { Authorization: `Bearer ${token}` } });
        console.log("Section created successfully:", response.data);
      }
      setShowSectionModal(false);
      // Refresh sections
      const res = await axios.get(`http://localhost:8080/api/course-sections/course/${courseId}`, { headers: { Authorization: `Bearer ${token}` } });
      setSections(res.data);
    } catch (error) {
  ...
  const handleDeleteSection = async (sectionId) => {
    ...
    const token = localStorage.getItem("token");
    await axios.delete(`http://localhost:8080/api/course-sections/${sectionId}`, { headers: { Authorization: `Bearer ${token}` } });
    // Refresh sections
    const res = await axios.get(`http://localhost:8080/api/course-sections/course/${courseId}`, { headers: { Authorization: `Bearer ${token}` } });
    setSections(res.data);
  };
  const moveSection = async (sectionId, direction) => {
    ...
    const token = localStorage.getItem("token");
    await axios.put(`http://localhost:8080/api/course-sections/course/${courseId}/reorder`, { sectionOrders: newOrder.map((s, i) => ({ sectionId: s._id, order: i })) }, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const handleLessonSubmit = async (e) => {
    ...
    if (lessonForm.lessonType === "file" && lessonFile) {
      ...
      const uploadRes = await axios.post(
        "http://localhost:8080/api/course-lessons/upload/file",
        uploadForm,
      ...
      await axios.post(
        "http://localhost:8080/api/course-lessons/",
        lessonData,
        { headers: { Authorization: `Bearer ${token}` } }
      );
    } else {
      ...
      if (editingLessonId) {
        await axios.put(`http://localhost:8080/api/course-lessons/${editingLessonId}`, dataToSend, config);
      } else {
        await axios.post(`http://localhost:8080/api/course-lessons/`, dataToSend, config);
      }
    }
    setShowLessonModal(false);
    setLessonFile(null);
    // Refresh lessons for the section
    const res = await axios.get(`http://localhost:8080/api/course-lessons/section/${lessonForm.sectionId}`, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const handleDeleteLesson = async (lessonId, sectionId) => {
    ...
    const token = localStorage.getItem("token");
    await axios.delete(`http://localhost:8080/api/course-lessons/${lessonId}`, { headers: { Authorization: `Bearer ${token}` } });
    // Refresh lessons for the section
    const res = await axios.get(`http://localhost:8080/api/course-lessons/section/${sectionId}`, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const moveLesson = async (sectionId, lessonId, direction) => {
    ...
    // Update order in backend
    const token = localStorage.getItem("token");
    await axios.put(`http://localhost:8080/api/course-lessons/section/${sectionId}/reorder`, { lessonOrders: newOrder.map((l, i) => ({ lessonId: l._id, order: i })) }, { headers: { Authorization: `Bearer ${token}` } });
