...
export default function StudentsPage() {
  ...
  const loadUnassignedStudents = async () => {
    ...
    try {
      const response = await axios.get(
        'http://localhost:8080/api/students/unassigned',
        { headers: getAuthHeaders() }
      );
  ...
  const handleClaimStudent = async (studentId: string) => {
    try {
      await axios.post(
        `http://localhost:8080/api/students/${studentId}/assign-self`,
        {},
        { headers: getAuthHeaders() }
      );
  ...
  const handleSubmitAssignment = async () => {
    ...
    try {
      const response = await axios.post(
        `http://localhost:8080/api/students/${selectedStudent._id}/assign`,
  ...
  const handleDeleteStudent = async (studentId: string) => {
    ...
    try {
      await axios.delete(`http://localhost:8080/api/students/${studentId}`, { headers: getAuthHeaders() });
