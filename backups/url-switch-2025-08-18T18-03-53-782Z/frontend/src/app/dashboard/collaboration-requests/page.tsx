...
export default function CollaborationRequestsPage() {
  ...
  const fetchAllRequests = async () => {
    try {
      ...
      const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/api/collaboration-requests/all`, {
  ...
  const handleStatusUpdate = async (requestId: string, status: 'approved' | 'rejected') => {
    try {
      const token = localStorage.getItem("token");
      await axios.put(
        `${process.env.NEXT_PUBLIC_API_URL}/api/collaboration-requests/${requestId}/status`,
        {
          status,
