...
console.log('🔄 Switching to local development URLs...');
console.log(`📝 Replacing: ${PRODUCTION_URL}`);
console.log(`📝 With: ${LOCAL_URL}`);
console.log('');
...
function replaceUrlsInFile(filePath) {
  try {
    ...
    // Replace the production URL with local URL
    content = content.replace(new RegExp(PRODUCTION_URL.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g'), LOCAL_URL);
    ...
    if (content !== originalContent) {
      ...
      // Count replacements
      const matches = originalContent.match(new RegExp(PRODUCTION_URL.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g'));
...
try {
  ...
  console.log('');
  console.log('🎉 Switching to local URLs completed!');
  ...
  console.log('');
  console.log('💡 To switch back to production, run: node scripts/switch-to-production.js');
